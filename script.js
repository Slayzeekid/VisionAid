...
let currentMode = null; // 'detection', 'navigation', 'both'
...
const switchCamera = async () => {
    ...
    if (previousMode === 'detection' || previousMode === 'both') {
        currentMode = previousMode; // Restore the mode
        statusText.textContent = "Camera switched. Starting detection...";
        startDetection();
    }
    if (previousMode === 'navigation' || previousMode === 'both') {
        currentMode = previousMode;
        // Re-prompt for destination to restart navigation
        speak("Camera switched. Where would you like to navigate now?");
    }
...
if (recognition) {
    recognition.onresult = (event) => {
        ...
        if (isWakeWordMode) {
            if (transcript.includes('hey vision') || transcript.includes('hi vision') || transcript.includes('hello vision')) {
                ...
                startSound.play();
                speak("Hello, what would you like to do? Detection, Navigation, or Both?", () => {
                    promptBox.style.display = 'block';
                });
            }
        } else {
            if (transcript.includes('detection')) {
            ...
            if (transcript.includes('navigation')) {
                ...
                // Check for specific destinations within the transcript
                ...
                // Only start navigation if a valid destination is found
                if (destination) {
                    ...
                    // If 'navigation' is said without a destination, or if 'both' is the mode
                    speak("Please specify a destination, like hospital, medical store, home, or garden.");
                    promptBox.style.display = 'block';
                }
            }
            if (transcript.includes('both')) {
                currentMode = 'both';
                speak("Starting both detection and navigation. Where would you like to go?");
                promptText.textContent = "Where would you like to go? hospital, medical store, home, or garden?";
                promptBox.style.display = 'block';
                startDetection();
            }
        }
    ...
    recognition.onerror = (event) => {
        ...
        if (event.error === 'network' || event.error === 'service-not-allowed') {
            statusText.textContent = "Error: Network issue or permission denied. Trying to reconnect.";
        }
        // This is a common error and should be handled to keep listening
